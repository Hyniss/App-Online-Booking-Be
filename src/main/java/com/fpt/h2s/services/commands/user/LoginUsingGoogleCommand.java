...
public class LoginUsingGoogleCommand implements BaseCommand<LoginUsingGoogleCommand.LoginUsingGoogleRequest, AuthResponse> {
    ...
    public ApiResponse<AuthResponse> execute(final LoginUsingGoogleRequest request) {
        final GoogleOAuth2User oauthUser = LoginUsingGoogleCommand.getGoogleUserUsingGoogleToken(request);
    ...
    private static GoogleOAuth2User getGoogleUserUsingGoogleToken(final LoginUsingGoogleRequest request) {
        try (final CloseableHttpClient httpClient = HttpClients.createDefault()) {
            final HttpPost httpPost = new HttpPost(LoginUsingGoogleCommand.GOOGLE_INFO_URL.formatted(request.getGoogleToken()));
    ...
    private User verifyUserIfPending(final User user) {
        ...
        if (user.is(PENDING)) {
            ...
            LoginUsingGoogleCommand.log.info("Verify user after login using Google.");
            return updatedUser;
        }
        return user;
    }
    ...
    private User createUserFrom(final GoogleOAuth2User googleOAuth2User) {
        ...
        LoginUsingGoogleCommand.log.info("Create user using Google successfully.");
        return newUser;
    }
